...
class RuntimeManager:
    ...
    def stop_and_delete_task(self, cid: str, kill_signal: int = 15, timeouts: Tuple[float,float] =
(3.0, 10.0)) -> None:
        """
        ...
        # Kill
        try:
            self.c.tasks.Kill(tasks_pb2.KillRequest(container_id=cid, signal=kill_signal), metadata
=ns_md(), timeout=timeouts[0])
        except grpc.RpcError as e:
        ...
        try:
            ...
            try:
                self.c.tasks.Kill(tasks_pb2.KillRequest(container_id=cid, signal=9), metadata=ns_md
(), timeout=timeouts[0])
                self.c.tasks.Delete(tasks_pb2.DeleteTaskRequest(container_id=cid), metadata=ns_md()
, timeout=timeouts[1])
            except grpc.RpcError:
                pass
...
class PodManager:
    ...
    def delete_pod(self, pod: Dict, apps: Optional[List[Dict]] = None) -> None:
        ...
        if pause_cid and network_name:
            ...
            try:
                print(f"[cleanup] CNI DEL network={network_name}, ifname={ifname}, netns={'present'
 if netns_for_del else 'missing'}")

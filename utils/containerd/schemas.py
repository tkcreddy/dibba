...
@dataclass
class ResourceSpec:
    cpu_millicores: int                # e.g., 500
    memory: str                        # e.g., "256Mi"
    cpuset_cpus: Optional[str] = None  # e.g., "0-1"

    def to_linux_resources_dict(self) -> dict:
        """
        Return a minimal OCI/Linux resources dict.
        We use period=100000 and quota = millicores * 100.
        """
        period = 100_000
        quota = int(self.cpu_millicores * 100) if self.cpu_millicores is not None else None

        # Convert "256Mi" to bytes (very simple parser; expand as needed)
        def _mem_to_bytes(s: Optional[str]) -> Optional[int]:
            if not s:
                return None
            ss = s.strip().lower()
            if ss.endswith("mi"):
                return int(ss[:-2]) * 1024 * 1024
            if ss.endswith("gi"):
                return int(ss[:-2]) * 1024 * 1024 * 1024
            if ss.endswith("m"):
                return int(ss[:-1]) * 1_000_000
            if ss.endswith("g"):
                return int(ss[:-1]) * 1_000_000_000
            # assume raw bytes
            return int(ss)

        mem_limit = _mem_to_bytes(self.memory)

        linux = {"resources": {}}
        if quota is not None:
            linux["resources"]["cpu"] = {"period": period, "quota": quota}
            if self.cpuset_cpus:
                linux["resources"]["cpu"]["cpus"] = self.cpuset_cpus

        if mem_limit is not None:
            linux["resources"]["memory"] = {"limit": mem_limit}
